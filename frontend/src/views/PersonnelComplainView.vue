    ...
    <el-container>
      <el-main>
        <div class="filter-container" style="display: flex; justify-content: space-between;height: 50px;">
          ...
          <div class="block">
            <el-date-picker v-model="pagination.date" type="daterange" range-separator="至" start-placeholder="开始日期"
        ...
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
          ...
          <el-table-column label="操作" width="300px" style="text-align: center;">
            <template #default="scope">
              <el-button type="primary" v-if="scope.row.status === '未受理'" @click="acceptRow(scope.row)">受理</el-button>
              <el-button type="success" v-if="scope.row.status === '已受理'" @click="completeRow(scope.row)">确认完成</el-button>
...
export default {
  ...
  methods: {
    ...
    updateComplaint() {
      // 发送请求以使用更新后的表单数据更新学员
      axios.put(`http://localhost:8081/complain/updateComplaint/${this.form.id}`, this.form, {
        headers: {
    ...
    updateStatus(id, status) {
      // 发送API请求更新数据库中的状态
      axios.put(`http://localhost:8081/personnel/updateComplainStatus/${id}/${status}`)
        .then(response => {
    ...
    getAll() {
      ...
      // 发送异步请求
      axios.get("http://localhost:8081/personnel/selectComplains/" + this.pagination.currentPage + '/' + this.pagination.pageSize + param).then((res) => {
